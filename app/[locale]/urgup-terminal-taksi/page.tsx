...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (locale !== 'tr') {
    return genMeta({
      ...
      path: '/urgup-terminal-taksi',
      noindex: true,
    });
  }
  
  const t = await getTranslations({ locale, namespace: 'urgupTerminalTaksi' });
...
export default async function UrgupTerminalTaksiPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  ...
  const canonicalUrl = buildLocaleUrl('tr', '/urgup-terminal-taksi');
