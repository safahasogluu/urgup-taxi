...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (locale !== 'tr') {
    return genMeta({
      ...
      path: '/urgup-taksi-duraklari',
      noindex: true,
    });
  }
  
  const t = await getTranslations({ locale, namespace: 'urgupTaksiDuraklari' });
...
export default async function UrgupTaksiDuraklariPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  ...
  const canonicalUrl = buildLocaleUrl('tr', '/urgup-taksi-duraklari');
