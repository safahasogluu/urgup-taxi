...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (locale !== 'tr') {
    return genMeta({
      ...
      path: '/urgup-taksi-numarasi',
      noindex: true,
    });
  }
  
  const t = await getTranslations({ locale, namespace: 'urgupTaksiNumarasi' });
...
export default async function UrgupTaksiNumarasiPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  
  // Only show for TR - use notFound() for others
  if (locale !== 'tr') {
    notFound();
  }
  ...
  const canonicalUrl = buildLocaleUrl('tr', '/urgup-taksi-numarasi');
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-8 pt-8 border-t border-zinc-200">
              <h3 className="font-display text-xl mb-4 text-basalt-900">İlgili Sayfalar</h3>
              <div className="flex flex-wrap gap-3">
                ...
                <Link
                  href="/tr/transfer/urgup-nav-transfer"
                  className="px-4 py-2 bg-amber-50 text-amber-800 rounded-lg hover:bg-amber-100 transition-colors font-medium"
                >
                  Ürgüp NAV Transfer
                </Link>
                <Link
                  href="/tr/transfer/urgup-asr-transfer"
                  className="px-4 py-2 bg-amber-50 text-amber-800 rounded-lg hover:bg-amber-100 transition-colors font-medium"
                >
                  Ürgüp ASR Transfer
                </Link>
