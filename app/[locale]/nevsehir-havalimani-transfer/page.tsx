...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (locale !== 'tr') {
    return genMeta({
      ...
      path: '/nevsehir-havalimani-transfer',
      noindex: true,
    });
  }
  
  const t = await getTranslations({ locale, namespace: 'nevsehirHavalimaniTransfer' });
...
export default async function NevsehirHavalimaniTransferPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  ...
  const canonicalUrl = buildLocaleUrl('tr', '/nevsehir-havalimani-transfer');
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-8 pt-8 border-t border-zinc-200">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link
                  href="/tr/transfer/goreme-nav-transfer"
                  className="px-4 py-2 bg-amber-50 text-amber-800 rounded-lg hover:bg-amber-100 transition-colors font-medium"
                >
                  Göreme NAV Transfer
                </Link>
